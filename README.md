...
For a significantly larger array size costs become more comparable
    ...
    `4.87$\mu$`s `$\pm$` `101ns` per loop (mean `$\pm$` std. dev. of 7 runs, 100000 loops each)
    
    In [14]: timeit ind = utf1st.find_1st(rr<limit, True, utf1st.cmp_equal)
    `8.95$\mu$`s `$\pm$` `497ns` per loop (mean $\pm$ std. dev. of 7 runs, 100000 loops each)
    
    In [15]: timeit ind = utf1st.find_1st(rr, limit, utf1st.cmp_smaller)
    `4.4$\mu$s $\pm$ 47.9ns` per loop (mean $\pm$ std. dev. of 7 runs, 100000 loops each)

Which demonstrates that even in this case the find_1st extension is more efficient
besides if the boolean intermediate array is used in line 14. In that case the comparison of the full array is 
actually performed twice.
...
This result is nevertheless a bit higher than expected from the sum of the simple boolean comparison
    ...
    `3.31$\mu$`s `$\pm$` 47.3ns per loop (mean `$\pm$` std. dev. of 7 runs, 100000 loops each)

and the find_1st operation in line 15 above, which would result in `7.7$\mu$s`  
